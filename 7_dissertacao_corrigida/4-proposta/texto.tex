...
\begin{enumerate}
  ...
  \item Obter a matriz de combinações lineares dos parâmetros do modelo que resultam nas médias ajustadas, denotada na \autoref{sec:multcomp} por $\boldsymbol{K_0}$.
  
  \item Gerar a matriz de contrastes (subtração duas a duas das linhas da matriz de combinações lineares), denotada na, denotada na \autoref{sec:multcomp} por $\boldsymbol{K_1}$.
