...
Com base na proposta de uso do teste de Wald para McGLMs, desenvolvemos o pacote \emph{htmcglm} com procedimentos para testar hipóteses lineares gerais, gerar quadros de ANOVA e MANOVA para parâmetros de regressão e dispersão e também testes de comparações múltiplas. Todos esses procedimentos foram implementados na linguagem R e complementam as funcionalidades existentes na biblioteca \emph{mcglm}.
...
O pacote \emph{mcglm} está disponível no Comprehensive R Archive Network (CRAN) em https://CRAN.R-project.org/package=mcglm. As implementações deste trabalho também foram submetidas e aceitas pelo CRAN e estão disponíveis em https://cran.r-project.org/package=htmcglm. Ambas as bibliotecas podem ser instaladas por meio da função \emph{install.packages()}.

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{install.packages}\hlstd{(}\hlstr{"mcglm"}\hlstd{)}
\hlkwd{install.packages}\hlstd{(}\hlstr{"htmcglm"}\hlstd{)}
