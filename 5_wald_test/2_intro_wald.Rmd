...
O teste Wald é um teste de hipóteses amplamente difundido para análises de modelos lineares e modelos lineares generalizados para verificar suposições sobre os parâmetros do modelo, isto é, verifcar se o parâmetro estimado é ou não estatísticamente igual a um valor qualquer.
...
O teste utiliza distribuição assintótica Qui-quadrado ( $\chi^2$ ) para verificar a validade das hipóteses e determinar significância estatística. Pode ainda ser generalizado para testar hipóteses a respeito dos parâmetros de um modelo multivariado como os modelos lineares multivariados (análogo ao modelo linear univariado) ou aos modelos multivariados de covariância linear generalizada, os McGLM (análogo aos modelos lineares generalizados). Além disso, é possível utilizar testes do tipo Wald para geração de quadros de análise de variância de modelos uni ou multivariados.
...
Em que $\beta$ pode representar um único ou vários parâmetros de regressão a serem testados simultaneamente e $\beta^*$ o valor ou vetor de valores com os quais desejamos verificar igualdade, isto é, sob hipótese nula.
...
Seu número de linhas é o número de parâmetros a serem testados ($s$) e o número de colunas é o mesmo que o número de parâmetros do modelo ($k$). $\beta$ é o vetor completo de parâmetros estimados no modelo, ou seja, é um vetor de $k$ elementos. E $c$ é um vetor de valores a serem confrontados com as estimativas originais do modelo, com dimensão $s$.

Sendo assim, $L$ tem dimensão $sxk$, $\beta$ tem dimensão $kx1$ e o produto $L\beta$ tem dimensão $sx1$, bem como $c$.
...
Em que $L$ é uma matriz de dimensão $sxk$, que define quais parâmetros estão sendo testados. $\beta$ é o vetor de dimensão $kx1$ com todos os parâmetros de regressão do modelo. $c$ é um vetor de dimensão $sx1$ com os valores sob hipótese nula. E $vcov(\hat\beta)$ é a matriz de variâncias e covariâncias das estimativas $\beta_j$ de dimensão $kxk$. 
...
Independente do número de parâmetros testados, a estatística de teste $W$ é um único valor que segue assintóticamente distribuição $\chi^2$. Quanto aos graus de liberdade, existem duas possibilidades: a primeira é considerar como graus de liberdade o número de parâmetros testados, isto é, o número de linhas da matriz $L$, denotado por $s$. A segunda possibilidade é utilizar a diferença entre o tamanho amostral e o número de parâmetros do modelo, ou seja, $N-k$.
...
1. Modelo com $x_1$, $x_2$ e interação vs modelo sem $x1$ e interação.
2. Modelo com $x_1$, $x_2$ e interação vs modelo sem $x2$ e interação.
